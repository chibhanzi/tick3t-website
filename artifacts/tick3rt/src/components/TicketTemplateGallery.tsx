...
export interface EventTemplate {
  ...
  textColor: string;
  gradient: string;
  editableObjects?: EditableTemplateObject[];
}

export type TemplateObjectKind = "category" | "overlay" | "accent";
export type TemplateObjectControl = "scale" | "scaleXY" | "color" | "opacity" | "rotation" | "radius";

export interface EditableTemplateObject {
  id: string;
  label: string;
  kind: TemplateObjectKind;
  controls?: TemplateObjectControl[];
}

const categoryObject: EditableTemplateObject = { id: "category", label: "Category tag", kind: "category", controls: ["scale", "color", "opacity"] };
const overlayObject: EditableTemplateObject = { id: "overlay", label: "Translucent layer", kind: "overlay", controls: ["scaleXY", "color", "opacity", "radius"] };
const accentObject = (id: string, label: string, controls: TemplateObjectControl[] = ["scale", "color", "opacity", "rotation", "radius"]): EditableTemplateObject =>
  ({ id, label, kind: "accent", controls });
...
const ElectricStagePreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(135deg, #0f0c29 0%, #302b63 55%, #24243e 100%)", fontFamily: "system-ui, sans-serif" }}>
    {/* Glow orb */}
    <div data-ticket-template-object="glow-orb" className="absolute top-[-20%] left-[15%] w-24 h-24 rounded-full opacity-30" style={{ background: "radial-gradient(circle, #a855f7, transparent)" }} />
    {/* Perforation */}
    ...
    {/* Bottom neon line */}
    <div data-ticket-template-object="neon-line" className="absolute bottom-0 left-0 right-0 h-0.5" style={{ background: "linear-gradient(90deg, transparent, #a855f7, transparent)" }} />
  </div>
);
...
const GoldenMicPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "#0a0800", fontFamily: "system-ui, sans-serif" }}>
    ...
    <div data-ticket-template-object="gold-triangle" className="absolute inset-0" style={{ clipPath: "polygon(55% 0%, 100% 0%, 100% 100%, 35% 100%)", background: "linear-gradient(160deg, #d4af37, #ffd700, #b8860b)" }} />
    {/* Left text area */}
    ...
    {/* Gold bottom border */}
    <div data-ticket-template-object="gold-border" className="absolute bottom-0 left-0 right-0 h-0.5" style={{ background: "#d4af37" }} />
  </div>
);
...
const IndieWavePreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(135deg, #f093fb 0%, #f5576c 55%, #fda085 100%)", fontFamily: "system-ui, sans-serif" }}>
    ...
    <div data-ticket-template-object="stamp-border" className="absolute inset-[4px] rounded-md border-2 border-dashed border-white/40" />
    {/* Center layout */}
    <div className="absolute inset-0 flex items-center px-5 gap-3">
      ...
      {/* Retro circle stamp */}
      <div data-ticket-template-object="circle-stamp" className="flex-shrink-0 w-14 h-14 rounded-full border-2 border-white/50 flex flex-col items-center justify-center text-center" style={{ background: "rgba(255,255,255,0.15)" }}>
    ...
    <svg data-ticket-template-object="wave-hem" className="absolute bottom-0 left-0 w-full" height="12" viewBox="0 0 400 12" preserveAspectRatio="none">
...
const GameDayPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "#111", fontFamily: "system-ui, sans-serif" }}>
    {/* Top green stripe */}
    <div data-ticket-template-object="top-stripe" className="absolute top-0 left-0 right-0 h-1" style={{ background: "#22c55e" }} />
    {/* Diagonal slash accent */}
    <div data-ticket-template-object="diagonal-slash" className="absolute top-0 right-[28%] bottom-0 w-10" style={{ background: "linear-gradient(160deg, transparent 48%, #22c55e22 48%, #22c55e22 52%, transparent 52%)" }} />
    {/* Left: event info */}
...
const PodiumPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", fontFamily: "system-ui, sans-serif", background: "#1a1a1a" }}>
    ...
    <div data-ticket-template-object="carbon-pattern" className="absolute inset-0" style={{ backgroundImage: "repeating-linear-gradient(45deg, #222 0px, #222 2px, transparent 2px, transparent 8px), repeating-linear-gradient(-45deg, #222 0px, #222 2px, transparent 2px, transparent 8px)", opacity: 0.8 }} />
    {/* Red racing stripe diagonal */}
    <div data-ticket-template-object="racing-stripe" className="absolute inset-0" style={{ background: "linear-gradient(160deg, transparent 40%, #ef4444 40%, #ef4444 52%, transparent 52%)" }} />
    {/* Left info */}
...
const SolsticePreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "radial-gradient(ellipse at 30% 50%, #ff6b6b 0%, #feca57 40%, #48dbfb 75%, #1dd1a1 100%)", fontFamily: "system-ui, sans-serif" }}>
    {/* Concentric rings */}
    <div data-ticket-template-object="outer-ring" className="absolute" style={{ right: "18%", top: "50%", transform: "translate(50%,-50%)", width: 80, height: 80, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.3)" }} />
    <div data-ticket-template-object="middle-ring" className="absolute" style={{ right: "18%", top: "50%", transform: "translate(50%,-50%)", width: 56, height: 56, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.4)" }} />
    <div data-ticket-template-object="inner-ring" className="absolute" style={{ right: "18%", top: "50%", transform: "translate(50%,-50%)", width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.5)" }} />
    {/* Left info */}
...
const NeonCarnivalPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "#050510", fontFamily: "monospace" }}>
    {/* Grid lines */}
    <div data-ticket-template-object="grid-lines" className="absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(0,245,255,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(0,245,255,0.06) 1px, transparent 1px)", backgroundSize: "20px 20px" }} />
    {/* Scan lines overlay */}
    <div data-ticket-template-object="scan-lines" className="absolute inset-0" style={{ backgroundImage: "repeating-linear-gradient(0deg, rgba(0,0,0,0.15) 0px, rgba(0,0,0,0.15) 1px, transparent 1px, transparent 4px)" }} />
    {/* Horizontal neon accent */}
    <div data-ticket-template-object="neon-accent" className="absolute left-0 right-0" style={{ top: "35%", height: 1, background: "linear-gradient(90deg, transparent, #00f5ff44, transparent)" }} />
    {/* Info */}
...
const SlateProPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", fontFamily: "system-ui, sans-serif" }}>
    {/* Left navy panel */}
    <div data-ticket-template-object="slate-panel" className="absolute inset-0 right-[38%]" style={{ background: "linear-gradient(135deg, #1e293b 0%, #334155 100%)" }} />
    {/* Right white panel */}
    <div data-ticket-template-object="white-panel" className="absolute inset-0 left-[62%] bg-white" />
    {/* Blue accent bar */}
    <div data-ticket-template-object="blue-bar" className="absolute left-0 top-0 bottom-0 w-1" style={{ background: "#3b82f6" }} />
    {/* Left text */}
...
const ExecutivePreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "#0a0a0a", fontFamily: "Georgia, serif" }}>
    ...
    {["top-1.5 left-1.5", "top-1.5 right-1.5", "bottom-1.5 left-1.5", "bottom-1.5 right-1.5"].map((pos, i) => (
      <div data-ticket-template-object={`corner-ornament-${i + 1}`} key={i} className={`absolute ${pos}`} style={{ width: 16, height: 16, borderColor: "#c0a060", borderStyle: "solid", borderWidth: i === 0 ? "2px 0 0 2px" : i === 1 ? "2px 2px 0 0" : i === 2 ? "0 0 2px 2px" : "0 2px 2px 0" }} />
    ))}
    {/* Fine horizontal lines */}
    <div data-ticket-template-object="top-rule" className="absolute left-4 right-[30%] top-4" style={{ height: 1, background: "linear-gradient(90deg, #c0a06033, #c0a060, #c0a06033)" }} />
    <div data-ticket-template-object="bottom-rule" className="absolute left-4 right-[30%] bottom-4" style={{ height: 1, background: "linear-gradient(90deg, #c0a06033, #c0a060, #c0a06033)" }} />
    {/* Center divider */}
    <div data-ticket-template-object="deco-divider" className="absolute top-4 bottom-4 right-[29%] w-px" style={{ background: "repeating-linear-gradient(to bottom, #c0a060 0px, #c0a060 4px, transparent 4px, transparent 8px)" }} />
    {/* Left text */}
...
const SummitPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", fontFamily: "system-ui, sans-serif" }}>
    ...
    {/* Diagonal band */}
    <div data-ticket-template-object="diagonal-band" className="absolute inset-0" style={{ background: "linear-gradient(160deg, transparent 45%, rgba(255,255,255,0.07) 45%, rgba(255,255,255,0.07) 55%, transparent 55%)" }} />
    {/* Left info */}
    <div className="absolute inset-0 right-[30%] flex flex-col justify-between p-3">
      <div>
...
const KeynotePreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(160deg, #005c97 0%, #003f6b 100%)", fontFamily: "system-ui, sans-serif" }}>
    ...
    <div data-ticket-template-object="ghost-number" className="absolute right-4 top-1/2 -translate-y-1/2 text-[72px] font-black leading-none select-none pointer-events-none" style={{ color: "rgba(255,255,255,0.04)", fontVariantNumeric: "tabular-nums" }}>01</div>
    {/* Top label bar */}
    <div data-ticket-template-object="label-bar" className="absolute top-0 left-0 right-0 h-5 flex items-center px-3 gap-2" style={{ background: "rgba(0,0,0,0.3)" }}>
...
const GalleryOpeningPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none border border-gray-200" style={{ aspectRatio: "7/3", background: "#fafaf9", fontFamily: "Georgia, serif" }}>
    ...
    <div data-ticket-template-object="left-stripe" className="absolute left-0 top-0 bottom-0 w-2" style={{ background: "#1a1a1a" }} />
    {/* Top hairline */}
    <div data-ticket-template-object="top-hairline" className="absolute top-4 left-6 right-6 h-px bg-gray-200" />
    {/* Bottom hairline */}
    <div data-ticket-template-object="bottom-hairline" className="absolute bottom-4 left-6 right-6 h-px bg-gray-200" />
    {/* Dashed vertical divider */}
    <div data-ticket-template-object="dashed-divider" className="absolute top-4 bottom-4 right-[28%]" style={{ width: 1, background: "repeating-linear-gradient(to bottom, #ccc 0px, #ccc 4px, transparent 4px, transparent 8px)" }} />
    {/* Main content */}
...
const AuroraPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(135deg, #43cea2 0%, #185a9d 50%, #8b5cf6 100%)", fontFamily: "system-ui, sans-serif" }}>
    ...
    {/* Flow curves */}
    <svg data-ticket-template-object="flow-curves" className="absolute inset-0 w-full h-full" viewBox="0 0 280 120" preserveAspectRatio="none">
...
const GalaNoirPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "#080808", fontFamily: "system-ui, sans-serif" }}>
    ...
    <div data-ticket-template-object="vertical-lines" className="absolute inset-0" style={{ backgroundImage: "repeating-linear-gradient(90deg, transparent 0px, transparent 18px, rgba(192,160,96,0.12) 18px, rgba(192,160,96,0.12) 19px)", backgroundSize: "19px 100%" }} />
    {/* Gold border frame */}
    <div data-ticket-template-object="outer-frame" className="absolute inset-[3px] rounded pointer-events-none" style={{ border: "1px solid rgba(192,160,96,0.3)" }} />
    <div data-ticket-template-object="inner-frame" className="absolute inset-[6px] rounded pointer-events-none" style={{ border: "1px solid rgba(192,160,96,0.1)" }} />
    {/* Divider */}
    <div data-ticket-template-object="gold-divider" className="absolute top-3 bottom-3 right-[28%]" style={{ width: 1, background: "linear-gradient(to bottom, transparent, #c0a060, transparent)" }} />
    {/* Content */}
...
const BlossomPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(135deg, #ffecd2 0%, #fcb69f 50%, #fbc2eb 100%)", fontFamily: "system-ui, sans-serif" }}>
    ...
    {[
      ...
      <div data-ticket-template-object={`flower-${i + 1}`} key={i} className="absolute rounded-full" style={{ width: c.s, height: c.s, top: c.t, left: c.l, transform: "translate(-50%,-50%)", background: `rgba(236,72,153,${c.o})`, border: `1px solid rgba(236,72,153,${c.o * 1.5})` }} />
    ))}
    {/* Wavy bottom edge SVG */}
    <svg data-ticket-template-object="blossom-hem" className="absolute bottom-0 left-0 w-full" height="10" viewBox="0 0 300 10" preserveAspectRatio="none">
...
const CybercorePreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "#030a03", fontFamily: "monospace" }}>
    {/* Hex dot pattern */}
    <div data-ticket-template-object="hex-grid" className="absolute inset-0" style={{ backgroundImage: "radial-gradient(circle, rgba(0,255,136,0.12) 1px, transparent 1px)", backgroundSize: "12px 12px" }} />
    {/* Horizontal scan bar */}
    <div data-ticket-template-object="scan-bar" className="absolute left-0 right-0" style={{ top: "45%", height: 2, background: "linear-gradient(90deg, transparent, rgba(0,255,136,0.3), transparent)" }} />
    {/* Corner bracket - top left */}
    <div data-ticket-template-object="corner-bracket-top" className="absolute top-2 left-2" style={{ width: 12, height: 12, borderTop: "2px solid #00ff88", borderLeft: "2px solid #00ff88" }} />
    {/* Corner bracket - bottom right (excluding stub area) */}
    <div data-ticket-template-object="corner-bracket-bottom" className="absolute bottom-2 left-2" style={{ width: 12, height: 12, borderBottom: "2px solid #00ff88", borderLeft: "2px solid #00ff88" }} />
    {/* Info */}
...
const HolographicPreview = ({
  ...
  <div className="relative w-full overflow-hidden rounded-lg select-none" style={{ aspectRatio: "7/3", background: "linear-gradient(135deg, #ff0080, #ff8c00, #ffff00, #40e0d0, #ee82ee, #9acd32)", fontFamily: "system-ui, sans-serif" }}>
    {/* Shimmer overlay */}
    <div data-ticket-template-object="shimmer" className="absolute inset-0" style={{ background: "repeating-linear-gradient(60deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.15) 10%, rgba(255,255,255,0) 20%)", backgroundSize: "200% 200%" }} />
...
export const TICKET_TEMPLATES: (EventTemplate & { Preview: React.FC<PreviewProps> })[] = [
  { id: "electric-stage",   name: "Electric Stage",    category: "Concert & Music",  description: "Neon-glow horizontal with purple stub and scan accents.",     isPremium: false, tags: ["music","concert","neon"],       primaryColor: "#a855f7", secondaryColor: "#0f0c29", accentColor: "#a855f7", textColor: "#ffffff", gradient: "linear-gradient(135deg, #0f0c29, #302b63)", editableObjects: [categoryObject, accentObject("glow-orb", "Glow orb"), accentObject("neon-line", "Neon line")], Preview: ElectricStagePreview },
  { id: "golden-mic",       name: "Golden Mic",        category: "Concert & Music",  description: "Diagonal gold triangle split on a black base, serif luxury.",  isPremium: true,  tags: ["music","jazz","gold"],           primaryColor: "#d4af37", secondaryColor: "#1a1200", accentColor: "#ffd700", textColor: "#ffffff", gradient: "linear-gradient(135deg, #1a1200, #d4af37)", editableObjects: [categoryObject, accentObject("gold-triangle", "Gold triangle"), accentObject("gold-border", "Gold border")], Preview: GoldenMicPreview },
  { id: "indie-wave",       name: "Indie Wave",        category: "Concert & Music",  description: "Wavy SVG bottom edge and retro stamp circle — festival poster vibe.", isPremium: false, tags: ["music","indie","retro"],   primaryColor: "#f5576c", secondaryColor: "#f093fb", accentColor: "#fda085", textColor: "#ffffff", gradient: "linear-gradient(135deg, #f093fb, #f5576c)", editableObjects: [categoryObject, accentObject("stamp-border", "Stamp border"), accentObject("circle-stamp", "Circle stamp"), accentObject("wave-hem", "Wave hem")], Preview: IndieWavePreview },
  { id: "game-day",         name: "Game Day",          category: "Sports",           description: "Dark stadium card with section/row/seat scoreboard stub.",      isPremium: false, tags: ["sports","match","stadium"],     primaryColor: "#22c55e", secondaryColor: "#111111", accentColor: "#22c55e", textColor: "#ffffff", gradient: "linear-gradient(135deg, #111, #1a2a1a)", editableObjects: [categoryObject, accentObject("top-stripe", "Top stripe"), accentObject("diagonal-slash", "Diagonal slash")], Preview: GameDayPreview },
  { id: "podium",           name: "Podium",            category: "Sports",           description: "Carbon-fibre weave background slashed by a red racing stripe.", isPremium: true,  tags: ["sports","racing","carbon"],     primaryColor: "#ef4444", secondaryColor: "#1a1a1a", accentColor: "#ef4444", textColor: "#ffffff", gradient: "linear-gradient(135deg, #232526, #414345)", editableObjects: [categoryObject, accentObject("carbon-pattern", "Carbon pattern"), accentObject("racing-stripe", "Racing stripe")], Preview: PodiumPreview },
  { id: "solstice",         name: "Solstice",          category: "Festival",         description: "Radial rainbow gradient with concentric ring decoration.",       isPremium: false, tags: ["festival","outdoor","summer"],  primaryColor: "#feca57", secondaryColor: "#ff6b6b", accentColor: "#1dd1a1", textColor: "#1a1a1a", gradient: "radial-gradient(ellipse, #ff6b6b, #feca57, #48dbfb)", editableObjects: [categoryObject, accentObject("outer-ring", "Outer ring"), accentObject("middle-ring", "Middle ring"), accentObject("inner-ring", "Inner ring")], Preview: SolsticePreview },
  { id: "neon-carnival",    name: "Neon Carnival",     category: "Festival",         description: "Dark grid + scan-line overlay with a cyan pixel-art stub.",     isPremium: true,  tags: ["festival","rave","cyber"],      primaryColor: "#00f5ff", secondaryColor: "#050510", accentColor: "#00f5ff", textColor: "#ffffff", gradient: "linear-gradient(135deg, #050510, #0d0d2a)", editableObjects: [categoryObject, accentObject("grid-lines", "Grid lines"), accentObject("scan-lines", "Scan lines"), accentObject("neon-accent", "Neon accent")], Preview: NeonCarnivalPreview },
  { id: "slate-pro",        name: "Slate Pro",         category: "Corporate",        description: "Navy/white split panel with blue accent bar — clean and sharp.", isPremium: false, tags: ["corporate","clean","split"],    primaryColor: "#3b82f6", secondaryColor: "#1e293b", accentColor: "#3b82f6", textColor: "#ffffff", gradient: "linear-gradient(135deg, #1e293b, #334155)", editableObjects: [categoryObject, accentObject("slate-panel", "Slate panel"), accentObject("white-panel", "White panel"), accentObject("blue-bar", "Blue accent bar")], Preview: SlateProPreview },
  { id: "executive",        name: "Executive",         category: "Corporate",        description: "Full black with art-deco gold corner ornaments and fine rules.", isPremium: true,  tags: ["corporate","luxury","deco"],    primaryColor: "#c0a060", secondaryColor: "#0a0a0a", accentColor: "#c0a060", textColor: "#ffffff", gradient: "linear-gradient(135deg, #0a0a0a, #1c1c1c)", editableObjects: [categoryObject, accentObject("corner-ornament-1", "Corner ornament 1"), accentObject("corner-ornament-2", "Corner ornament 2"), accentObject("corner-ornament-3", "Corner ornament 3"), accentObject("corner-ornament-4", "Corner ornament 4"), accentObject("top-rule", "Top rule"), accentObject("bottom-rule", "Bottom rule"), accentObject("deco-divider", "Deco divider")], Preview: ExecutivePreview },
  { id: "summit",           name: "Summit",            category: "Conference",       description: "Purple gradient with translucent circle and diagonal band.",    isPremium: false, tags: ["conference","tech","modern"],   primaryColor: "#667eea", secondaryColor: "#764ba2", accentColor: "#a78bfa", textColor: "#ffffff", gradient: "linear-gradient(135deg, #667eea, #764ba2)", editableObjects: [categoryObject, overlayObject, accentObject("diagonal-band", "Diagonal band")], Preview: SummitPreview },
  { id: "keynote",          name: "Keynote",           category: "Conference",       description: "Bold blue with oversized ghost numeral and labelled gate stub.", isPremium: false, tags: ["conference","keynote","bold"],  primaryColor: "#005c97", secondaryColor: "#003f6b", accentColor: "#60a5fa", textColor: "#ffffff", gradient: "linear-gradient(160deg, #005c97, #003f6b)", editableObjects: [categoryObject, accentObject("ghost-number", "Ghost number"), accentObject("label-bar", "Label bar")], Preview: KeynotePreview },
  { id: "gallery-opening",  name: "Gallery Opening",  category: "Art & Culture",    description: "Bright white with a thick left stripe — gallery card minimalism.", isPremium: false, tags: ["art","gallery","minimal"],   primaryColor: "#1a1a1a", secondaryColor: "#fafaf9", accentColor: "#1a1a1a", textColor: "#1a1a1a", gradient: "linear-gradient(135deg, #fafaf9, #f0ede8)", editableObjects: [categoryObject, accentObject("left-stripe", "Left stripe"), accentObject("top-hairline", "Top hairline"), accentObject("bottom-hairline", "Bottom hairline"), accentObject("dashed-divider", "Dashed divider")], Preview: GalleryOpeningPreview },
  { id: "aurora",           name: "Aurora",            category: "Art & Culture",    description: "Teal-to-purple gradient with flowing SVG curves and glass stub.", isPremium: true, tags: ["art","theatre","flowing"],     primaryColor: "#43cea2", secondaryColor: "#185a9d", accentColor: "#43cea2", textColor: "#ffffff", gradient: "linear-gradient(135deg, #43cea2, #185a9d, #8b5cf6)", editableObjects: [categoryObject, overlayObject, accentObject("flow-curves", "Flow curves")], Preview: AuroraPreview },
  { id: "gala-noir",        name: "Gala Noir",         category: "Charity & Gala",   description: "Black with vertical gold art-deco lines and a double-frame border.", isPremium: true, tags: ["gala","charity","deco"],  primaryColor: "#c0a060", secondaryColor: "#080808", accentColor: "#c0a060", textColor: "#ffffff", gradient: "linear-gradient(135deg, #0a0a0a, #1a1a1a)", editableObjects: [categoryObject, accentObject("vertical-lines", "Vertical lines"), accentObject("outer-frame", "Outer frame"), accentObject("inner-frame", "Inner frame"), accentObject("gold-divider", "Gold divider")], Preview: GalaNoirPreview },
  { id: "blossom",          name: "Blossom",           category: "Charity & Gala",   description: "Soft peach gradient with floral circles and wavy SVG hem.",      isPremium: false, tags: ["gala","garden","delicate"],  primaryColor: "#ec4899", secondaryColor: "#ffecd2", accentColor: "#ec4899", textColor: "#4a1942", gradient: "linear-gradient(135deg, #ffecd2, #fcb69f, #fbc2eb)", editableObjects: [categoryObject, accentObject("flower-1", "Large flower"), accentObject("flower-2", "Medium flower"), accentObject("flower-3", "Small flower"), accentObject("blossom-hem", "Blossom hem")], Preview: BlossomPreview },
  { id: "cybercore",        name: "Cybercore",         category: "Tech & Gaming",    description: "Black with hex dot grid, green scan-line and corner brackets.",  isPremium: false, tags: ["gaming","esports","hex"],       primaryColor: "#00ff88", secondaryColor: "#030a03", accentColor: "#00ff88", textColor: "#ffffff", gradient: "linear-gradient(135deg, #030a03, #001a0a)", editableObjects: [categoryObject, accentObject("hex-grid", "Hex grid"), accentObject("scan-bar", "Scan bar"), accentObject("corner-bracket-top", "Top corner bracket"), accentObject("corner-bracket-bottom", "Bottom corner bracket")], Preview: CybercorePreview },
  { id: "holographic",      name: "Holographic",       category: "Tech & Gaming",    description: "Rainbow iridescent with glassmorphism info card floating above.", isPremium: true, tags: ["tech","rainbow","glass"],      primaryColor: "#ff0080", secondaryColor: "#40e0d0", accentColor: "#40e0d0", textColor: "#ffffff", gradient: "linear-gradient(135deg, #ff0080, #ff8c00, #40e0d0, #ee82ee)", editableObjects: [categoryObject, overlayObject, accentObject("shimmer", "Shimmer")], Preview: HolographicPreview },
];
