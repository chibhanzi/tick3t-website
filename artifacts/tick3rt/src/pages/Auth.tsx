...
const Auth = () => {
  ...
  const handleSignIn = async (e: React.FormEvent): Promise<void> => {
    ...
    if (!emailParsed.success) { toast({ title: "Invalid email", description: emailParsed.error.errors[0].message, variant: "destructive" }); return; }
  ...
  const handleSignUp = async (e: React.FormEvent): Promise<void> => {
    ...
    if (firstErr && !firstErr.success) {
      toast({ title: "Check your details", description: firstErr.error.errors[0].message, variant: "destructive" });
      return;
    }
  ...
  const handleReset = async (e: React.FormEvent): Promise<void> => {
    ...
    if (!parsed.success) { toast({ title: "Invalid email", variant: "destructive" }); return; }
    setLoading(true);
    const { error } = await requestPasswordReset(parsed.data);
    setLoading(false);
    if (error) { toast({ title: "Could not send reset email", description: error, variant: "destructive" }); return; }
    setResetSent(true);
  };
